...
export const VideoPlayer = forwardRef<VideoPlayerHandle, VideoPlayerProps>(function VideoPlayer(
  ...
  const qualityLabel = useMemo(() => {
    // On an HLS source the button shows the active bitrate rung
    // (Auto / 1080p / 720p…). On a non-HLS source (e.g. the Original
    // direct file) it shows the selected source label.
    if (isHls) {
      if (selectedQualityLevel === AUTO_QUALITY_LEVEL) return "Auto";
      return (
        qualityOptions.find((option) => option.level === selectedQualityLevel)
          ?.label ?? "Auto"
      );
    }
    if (hasExternalQualityOptions) {
      return (
        qualityOptionsConfig?.find((option) => option.id === selectedQualityId)
          ?.label ?? "Original"
      );
    }
    return "Original";
  }, [isHls, qualityOptions, hasExternalQualityOptions, qualityOptionsConfig, selectedQualityId, selectedQualityLevel]);
            ...
            {qualityMenuOpen && (
              ...
              >
                {/* Bitrate ladder — the adaptive HLS rungs. Shown
                    whenever the current source is HLS, so users can
                ...
                {/* Source switch — proxy vs original full-res, etc.
                    Supplied by the parent. Lives alongside the bitrate
                    ladder so a single menu drives both. */}
